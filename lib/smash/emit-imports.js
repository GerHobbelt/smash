...
module.exports = function(file) {
  ...
  function line(line, number) {
    if (/^\s*(import|<include)\b/.test(line)) {
      flush();
      var match = /^\s*import\s+["']([^"']+)["']\s*;?\s*(?:\/\/.*)?$/.exec(line);
	    if(!match){
		    match = /^\s*<include\s+src\s*=\s*["']([^"']+)["']\/?>\s*(?:\/\/.*)?$/.exec(line);
	    }
      if (match) {
  ...
  function flush() {
    if (lines.length){
	    // append imported file's name info
	    // TODO : configuration support.
	    lines.unshift(new Buffer("//#Import Source File => " + file + " Start.#\n\n"));
